 ...
 * - GOOGLE_SCRIPT_CONTACT_URL: The Google Apps Script Web App URL for contact form
 */
...
export default async function handler(
  ...
  const googleScriptUrl = process.env.GOOGLE_SCRIPT_CONTACT_URL;

  if (!googleScriptUrl) {
    console.error('GOOGLE_SCRIPT_CONTACT_URL environment variable is not set');
  ...
  try {
    ...
    if (googleResponse.ok) {
      ...
      return response.status(googleResponse.status).json({
        success: false,
        error: parsedData.error || 'Failed to submit contact form',
        details: parsedData
      });
    }
  } catch (error) {
